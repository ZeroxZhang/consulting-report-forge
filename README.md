...
组件会检查部分数据和容量约束。没有现成组件时，可以使用自定义 SVG；自定义内容仍需接受实际预览和审查。
...
制作按“蓝图校验 → 逐页装配与预览 → 高风险页复核 → 最终验收与打包”推进。自动检查覆盖布局、部分文字与图表约束、关键内容、字体和最终输出等工程问题；视觉审查与独立复核负责机器规则无法判断的论证与可读性。

**自动通过不等于商业判断正确。** 技能会区分有脚本强制检查的规则与需要作者、复核者判断的要求。

## 布局系统

**30 套正文布局，把页面结构变成论证的一部分。** 核心判断放在哪里，证据怎样展开，条件如何与结论对应，行动如何与责任连接，都有适合的组织方式。
...
- **用统一网格控制交付质量。** 正文采用 12×6 网格，支持 16:9 与 4:3；模块位置、内容职责和文字容量有明确约定，最终页面还会检查实际落位。
...
没有 Chrome 时可用 `npx playwright install chromium` 安装浏览器，并为后续制稿命令设置 `CHROME_CHANNEL=chromium`。首次运行请让 Agent 完成环境及读图探测，具体步骤见[安装与环境探测](references/static-html-pdf.md#安装与环境探测)。
...
- `research/` 是本项目保留的研究示例，并非持续更新的数据服务；使用其中数据前要确认来源与时效。
...
维护者可运行 `npm test` 检查主要合同与生成文件一致性，运行 `npm run test:render` 检查浏览器回归。字体缓存专项检查为 `.font-venv/bin/python scripts/test_font_cache.py`。这些检查各有范围，完整报告仍需走自己的最终验收。
