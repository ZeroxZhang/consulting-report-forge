...
仅记录会改变判断的主张，放在蓝图已有的 `sourcePlan.claims`；不另建重复台账。正文、关键限定和来源索引引用同一份记录。机器只核对字段、引用和待核实状态，来源真假、口径是否可比及推理是否成立仍由作者与独立复核判断。
...
- `sourceKeys`：必须引用本页`sourcePlan.keys`里实际可定位的URL或文件/行号；事实、估计和预测不可为空。
...
新报告按本记录执行；旧蓝图未登记claims仍可读取。一旦登记，蓝图校验会检查结构和引用，`--ready`拒绝残留的pending。重大报告画页前的独立复核检查：跨源拼接、分母、预测/假设身份、研究样本外推、条件树分支；最终仍审查实际HTML/PDF，前置复核不能预填最终PASS。
