...
```sh
node scripts/snapshot_review.cjs /任务/renders/audit.json /任务/renders/review.json /任务/snapshots/v1
```
...
```sh
node scripts/prepare_review_reuse.cjs /任务/renders/audit.json /任务/snapshots/v1 /任务/reuse-v2
```
...
`task.pages` 和 `task.blueprint` 的整文件摘要不用于整册失效；它们仍与 HTML 合同及实际来源文件核对。新版每页 `data-content-hash` 随内容变化进入页面摘要。**这不意味着其他页的商业结论一定仍成立**：总判断、决策条件或跨页推理的变化必须在本轮全局复核中检查。
...
```sh
node scripts/aggregate_reviews.cjs /任务/renders/audit.json /任务/renders/review.json /任务/reuse-v2/author-1.json /任务/reuse-v2/independent-1.json
node scripts/snapshot_review.cjs /任务/renders/audit.json /任务/renders/review.json /任务/snapshots/v2
```

聚合仍强制作者/独立角色与整册 HTML/PDF 覆盖，不因复用降低标准。没有看过的新页、只覆盖一种媒介、当前文件摘要不同、遗漏旧未决问题，都不能成为完成的审查。

## 历史单人审查与边界

推荐以完整快照的汇总 review 作为来源，工具已按原身份自动选取覆盖。兼容历史单人或分批结果时，可在对应 `inheritedFrom` 显式写 `reviewScope: "partial"`：递归验证仅按该原始审查者的身份及覆盖范围核验；默认仍是 `complete`。这不放宽当前整册覆盖，也不允许将单人文件直接归档成完整快照。
