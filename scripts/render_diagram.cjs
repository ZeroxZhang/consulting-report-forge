...
const diagramContract=require('./diagram_contract.cjs');
...
function render(spec){
  // 历史独立图示未声明 form；新蓝图的最终对账会拒绝没有 data-form 的主图。
  if(spec.form!==undefined)spec=diagramContract.prepare(spec);
  ...
  function label(text,x,y,{size=16,weight=400,fill=palette.ink,anchor='start',role,backplate=false}={}){
    ...
    const plateX=Math.max(0,left-5),plateRight=Math.min(width,left+w+5);
    const plate=backplate?`<rect x="${plateX}" y="${Math.max(0,y-size-5)}" width="${plateRight-plateX}" height="${Math.min(height, y+5)-Math.max(0,y-size-5)}" fill="${palette.bg||'#FFFFFF'}" data-role="edge-label-backplate"/>`:'';
    return plate+`<text x="${x}" y="${y}" font-size="${size}" font-weight="${weight}" text-anchor="${anchor}" fill="${fill}"${role?' data-role="'+escape(role)+'"':''}>${escape(text)}</text>`;
  }
  ...
  const groupSVG=groups.map(g=>`<g${g.role?' data-role="'+escape(g.role)+'"':''}><rect x="${g.x}" y="${g.y}" width="${g.w}" height="${g.h}" rx="${g.radius??0}" fill="${color(g.fill,palette.surface)}" stroke="${color(g.stroke,palette.grid)}"/>${g.title?label(g.title,g.x+14,g.y+26,{size:18,weight:600}):''}</g>`).join('');
  ...
  const edgeSVG=(spec.edges||[]).map((e,i)=>{
    ...
    const stroke=color(e.color,palette.muted),strokeWidth=number(e.width??diagramContract.EDGE_WIDTH,'线宽');if(strokeWidth<=0)throw Error('线宽必须为正');
    const center=points[Math.floor((points.length-1)/2)],next=points[Math.ceil((points.length-1)/2)];
    return `<g data-edge="${escape(e.id||e.from+'--'+e.to)}" data-from="${escape(e.from)}" data-to="${escape(e.to)}"${e.role?' data-edge-role="'+escape(e.role)+'"':''}${e.relation?' data-relation="'+escape(e.relation)+'"':''}${e.evidenceStatus?' data-evidence-status="'+escape(e.evidenceStatus)+'"':''}${e.feedback?' data-feedback="true"':''}${e.condition?' data-condition="'+escape(e.condition)+'"':''}${e.probability!==undefined?' data-probability="'+escape(e.probability)+'" data-probability-basis="'+escape(e.probabilityBasis)+'"':''}><polyline points="${points.map(p=>p.join(',')).join(' ')}" fill="none" stroke="${stroke}" stroke-width="${strokeWidth}"${e.dashed?' stroke-dasharray="6 4"':''}${e.arrow===false?'':` marker-end="url(#${id}-arrow)"`}/>${e.labelBox?`<rect x="${e.labelBox.x}" y="${e.labelBox.y}" width="${e.labelBox.w}" height="${e.labelBox.h}" fill="${palette.bg||'#FFFFFF'}"/>`:""}${e.label?label(e.label,e.labelX??(center[0]+next[0])/2,e.labelY??(center[1]+next[1])/2-10,{size:e.fontSize??14,anchor:'middle',fill:stroke,backplate:!e.labelBox}):''}</g>`;
  }).join('');
  const nodeSVG=nodes.map(n=>{
    ...
    return `<g data-node="${escape(n.id)}"${n.role?' data-node-role="'+escape(n.role)+'"':''}${n.lane?' data-lane="'+escape(n.lane)+'"':''}${n.stage!==undefined?' data-stage="'+escape(n.stage)+'"':''}>${geometry}${texts}</g>`;
  }).join('');
  const annotations=(spec.annotations||[]).map(a=>label(a.text,a.x,a.y,{size:a.fontSize??14,weight:a.weight??400,fill:color(a.color,palette.muted),anchor:a.anchor||'start',role:a.role})).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="${id}-title" data-diagram="${id}"${spec.form?' data-form="'+escape(spec.form)+'" data-diagram-contract="1"':''} style="font-family:${escape(type.body)};font-synthesis:none;text-rendering:geometricPrecision;font-variant-numeric:tabular-nums"><title id="${id}-title">${escape(spec.title||'关系图')}</title><defs><marker id="${id}-arrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="context-stroke"/></marker></defs>${groupSVG}${edgeSVG}${nodeSVG}${annotations}</svg>`;
}
