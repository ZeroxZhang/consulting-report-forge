...
const {fixture,task:legacyTask}=require('../tests/fixtures/analysis_fixture.cjs'),analysis=require('./analysis_contract.cjs'),content=require('./content_contract.cjs'),contract=require('./report_contract.cjs'),evidence=require('./audit_evidence.cjs'),review=require('./review_contract.cjs'),snap=require('./snapshot_review.cjs');
for(const strict of [false,true]){
const task=strict?{...legacyTask,version:3,analysisAlgorithm:'semantic-v2'}:legacyTask;
...
const d=fixture();if(strict)d.analysisAlgorithm='semantic-v2';write('input.csv','value\n120\n');d.artifacts=[{id:'input-file',path:'input.csv',kind:'input',sha256:contract.fileHash(path.join(live,'input.csv')),dependsOn:[]}];d.analysis.workItems[0].artifactRefs=['input-file'];
const ar={schemaVersion:strict?2:1,...(strict?{status:'complete',analysisAlgorithm:'semantic-v2',analysisProjection:require('./analysis_projection.cjs').project(d).projection}:{}),analysisSha256:analysis.digest(d,task),reviews:[{role:'author',reviewer:'fixture',instanceId:'fixture-instance',conclusion:'ready',basis:'校验器夹具',coverage:{claimRefs:d.claims.map(c=>c.id),issueRefs:['revenue-issue'],optionRefs:[],...(strict?{slideRefs:d.slides.map(s=>s.id)}:{})}}],issues:[]};write('analysis-review.json',ar);const t={...task,analysisReview:{record:'analysis-review.json',sha256:contract.fileHash(path.join(live,'analysis-review.json'))}};
...
const r={schemaVersion:strict?5:4,...(strict?{analysisAlgorithm:'semantic-v2'}:{}),status:'complete',reviewer:'fixture',independence:'author',analysisSha256:analysis.digest(d,task),htmlSha256:artifacts.html.sha256,pdfSha256:artifacts.pdf.sha256,auditSha256:contract.hash(contract.stable(audit)),coverage:[{reviewer:'fixture',independence:'author',layers:review.layers,htmlPages:[1],pdfPages:[1],evidence:manifest.entries.map(e=>({id:e.id}))}],checks:{analysis:{status:'pass',basis:'夹具'},evidence:{status:'pass',basis:'夹具'},visual:{status:'pass',basis:'夹具'}},warningReview:[],issues:[]};write('review.json',r);
assert.deepEqual(review.validate(r,audit,{baseDir:live}),[]);
const target=path.join(root,'snapshot');snap.snapshotReview({auditFile:path.join(live,'audit.json'),reviewFile:path.join(live,'review.json'),outputDir:target});fs.rmSync(live,{recursive:true,force:true});assert.equal(snap.loadSnapshot(target).review.schemaVersion,strict?5:4);
const loaded=snap.loadSnapshot(target);
const aggregate=require('./aggregate_reviews.cjs').aggregate(loaded.audit,[loaded.review],{baseDir:path.dirname(loaded.reviewFile),auditDir:path.dirname(loaded.auditFile)});assert.equal(aggregate.status,'complete',JSON.stringify(aggregate.aggregationErrors));
const prepared=require('./prepare_review_reuse.cjs').prepareReuse({auditFile:loaded.auditFile,snapshotDir:target,outputDir:path.join(root,'reuse')});
const draft=JSON.parse(fs.readFileSync(path.join(prepared.directory,prepared.drafts[0])));assert.equal(draft.schemaVersion,strict?5:4);assert.equal(draft.status,'incomplete');assert.equal(draft.analysisSha256,null);
...
console.log('PASS analysis snapshot: legacy4/strict5 portable snapshots, aggregation, incomplete reuse, analysis/visual separation');
